...
const Slider = ({ children, handlePresent, present, call }) => {
  ...
  const handleSlidePrevTransitionEnd = (swiper) => {
    if (sliderHeight) {
      console.log("حضور");
      handlePresent();
      // Reset to middle slide when swiping
      swiper.slideTo(1, 1000, false); // Always reset to middle slide (index 1)
    }
  };
    ...
    <div className="w-full mx-auto " style={{ height: `${sliderHeight}px` }}>
      ...
      >
        <SwiperSlide className="h-32">
          <div
            className={`${
              present ? "bg-red-300" : "bg-green-300"
            } text-2xl font-extrabold text-white rounded-xl h-full flex items-center px-5 justify-end`}
          >
            {present ? "غایب" : "حاضر"}
          </div>
        </SwiperSlide>
